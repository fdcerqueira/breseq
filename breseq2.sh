...
#all fastq files will have the prefix Inv/Res
...
#genones parder
download_genomes=$SCRIPT_DIR/parse_taxa.py
...
#loop to create options to be chosen. i.e. 1.invader.., 2.res...
echo ""
...
#while loop with case to allow to choose parameters in for the downstream programs
while [ ! -z "$1" ]; do
    case "$1" in
    -h) show_help ;exit ;;
    ...
    -b) shift 1; MODE=$1;  echo "the breseq mode selected was: ${MODE}"; echo "";;
    -r) shift 1; REFERENCE=$1; filename=$(basename -- "${file[$REFERENCE]}"); filename1="${filename%%.*}"; echo "the reference selected selected was: ${filename1}";echo "";;
...
#in case single end mode is prefered, the merged reads from several lanes will be input for flash to join the pair-end reads, otherwise just copy files
if [ "$single" = "y" ]
then
...
#run bbsplit function
for prefix in $(ls $processed_fastq/*.fastq.gz | sed -E "s/_R[12]_001[.]fastq.gz//" | uniq) 
do
...
#if last command sucessfull write:
if [ $? -eq 0 ] 
then
    echo "#########################################################################"
    echo "#                                                                       #"
